...
def _collect_uniform_pipeline_status(
    ...
    for accession_id in accession_ids:
        try:
            analysis_set = cast(
                AnalysisSet, api.get_by_id(accession_id).actual_instance
            )
        except ValueError:
...
class AnnotationsFileQc:
    ...
    def qc(api: IgvfApi, annotations_file_accession: str | None) -> AnnotationsFileQc:
        ...
        try:
            ...
            raise ValueError(
                f"Error QC-ing {annotations_file_accession}"
            ) from exception
