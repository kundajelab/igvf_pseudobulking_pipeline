...
PSEUDOBULK_FILE_DEFINITIONS: Final[tuple[IgvfUploadBase, ...]] = (
    ...
    TabularFile(
        ...
        content_type="peaks",
        file_format_specifications="TODO: fix",
    ),
    TabularFile(
...
class FileUploadRows:
    tabular_rows: list[dict[str, str]] = dataclasses.field(default_factory=list)
    matrix_rows: list[dict[str, str]] = dataclasses.field(default_factory=list)
    signal_rows: list[dict[str, str]] = dataclasses.field(default_factory=list)
...
class UploadState:
    basedir: Path
    config: Config
    document_rows: list[dict[str, str]] = dataclasses.field(default_factory=list)
    ...
    def _write_tsv(
        ...
        return f'iu_register "$dry_run_arg" -m "$igvf_mode" -p {upload_type} -i "{self.upload_tsvs_dir.name}/{outfile_name}"'
    ...
    def write_upload_state(self) -> None:
        ...
        if len(self.document_rows) > 0:
            submit_command = self._write_tsv(
                "document", self.document_rows, analysis_step=None
            )
            self.submission_rows.append(submit_command)
        # 2. Pseudobulks
        if len(self.pseudobulk_folders_docs) > 0:
            ...
            submit_command = self._write_tsv(
                "pseudobulk_set", pseudobulk_rows, analysis_step=None
            )
            self.submission_rows.append(submit_command)
        ...
        self.submission_rows.append("echo upload succeeded.")
