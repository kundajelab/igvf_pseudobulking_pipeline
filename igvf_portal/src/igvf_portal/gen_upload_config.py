...
class GenUploadConfig:
        ...
        if self.input_accessions is None:
            ...
            return {
                ...
                for fragment_accession in self._lookup_content_accessions(
                    self.lookup_record(file_set)["accession"], content_type=content_type
                )
            }
        else:
    ...
    def reference_ids(self) -> set[PortalId]:
        ...
        return {
            ...
            for ref_id in self._lookup_aligned_refs(aligned_accession)
        }
    ...
    @cached_property
    def input_fragments_portal_ids(self) -> list[PortalId]:
    ...
    @cached_property
    def input_matrices_portal_ids(self) -> list[PortalId]:
    ...
    @cached_property
    def file_set_fragments_portal_ids(self) -> Mapping[PortalId, frozenset[PortalId]]:
        """Get mapping from input file_set ID to set of input fragments IDs."""
        return self._collect_portal_ids_by_file_set(self.input_fragments_portal_ids)

    @cached_property
    def file_set_matrices_portal_ids(self) -> Mapping[PortalId, frozenset[PortalId]]:
        """Get mapping from input file_set ID to set of input matrix IDs."""
        return self._collect_portal_ids_by_file_set(self.input_matrices_portal_ids)
    ...
    def derived_from(
        ...
        match analysis_step, output_category:
            case AnalysisStep.PSEUDOBULK_ATAC_SEQ, OutputCategory.PSEUDOBULK:
                annotations_row = self.get_annotations_row(pseudobulk_path=upload_path)
                input_ids = {
                    portal_id
            ...
            case AnalysisStep.PSEUDOBULK_RNA_SEQ, OutputCategory.PSEUDOBULK:
                ...
                input_ids = {
                    portal_id
                    for file_set in self.get_input_file_sets(annotations_row)
                    for portal_id in self.file_set_matrices_portal_ids.get(file_set, [])
                }
                derived_from_ids = sorted(input_ids | annotations_id_list)
                ...
                derived_from_ids = sorted(
                    {
                        ...
                        for input_aliases in (
                            self.input_fragments_portal_ids,
                            self.input_matrices_portal_ids,
                            annotations_id_list,
                        )
    ...
    @cached_property
    def file_sets(self) -> list[PortalId]:
        ...
        if self.input_file_sets is None or len(self.input_file_sets) == 0:
            ...
            return sorted(
                self.lookup_record(Alias(file_set))["@id"]
                for file_set in self.input_file_sets.split(",")
            )
